...
public class PaymentInitiatedConsumer : IConsumer<PaymentInitiatedEvent>
{
    ...
    public async Task Consume(ConsumeContext<PaymentInitiatedEvent> context)
    {
        ...
        var existingPayment = await _paymentRepository.GetSuccessfulPaymentByOrderId(order.OrderId);
        ...
        var paymentRecord = new Payment(order.OrderId, order.OrderTotal);
        ...
        try
        {
            ...
            if (paymentResult.IsSuccess)
            {
                ...
                await context.Publish(new PaymentCompletedEvent { OrderId = order.OrderId });
            }
            else
            {
                paymentRecord.MarkFailed(paymentResult.ErrorMessage);
                await _paymentRepository.UpdateAsync(paymentRecord);
                _logger.LogError($"Payment processing failed for OrderId: {order.OrderId}. Error: {paymentResult.ErrorMessage}");
                await context.Publish(new PaymentFailedEvent { OrderId = order.OrderId, ErrorMessage = paymentResult.ErrorMessage });
            }
        }
        catch (Exception ex)
        {
            ...
            _logger.LogError(ex, $"An error occurred while processing payment for OrderId: {order.OrderId}");
            await context.Publish(new PaymentFailedEvent { OrderId = order.OrderId, ErrorMessage = "An error occurred while processing the payment." });
    ...
    {
        return new SaleTransaction(order.OrderId, DateTime.UtcNow, order.OrderTotal, paymentId);
    }
}
